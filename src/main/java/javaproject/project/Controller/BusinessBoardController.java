...
//===== 업체링크 시작
    ...
    public String business_Board_Map(Model model) {
        List<User_T> userList = this.userRepository.findAll();
        List<Board> boardList = this.boardRepository.findByBoardMap(31);
        ...
        // Comparator를 사용하여 boardList를 author.id 기준으로 정렬
    ...
    public String modifyBoardMap(Model model, @PathVariable("id") Integer id, BoardCreateForm boardCreateForm) {
        Board board = this.boardService.getBoard(id);
        boardCreateForm.setTitle(board.getTitle());
        boardCreateForm.setContent(board.getContent());
